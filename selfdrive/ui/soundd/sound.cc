...
void Sound::update() {
  ...
  if (sm.updated("carState")) {
    float volume = util::map_val(sm["carState"].getCarState().getVEgo(), 11.f, 30.f, 0.f, 1.0f); // bensonkwek max volume at ~65mph (referencing KRKeegan)
    volume = QAudio::convertVolume(volume, QAudio::LogarithmicVolumeScale, QAudio::LinearVolumeScale);
    float max_volume_scale = 1.2;  //  percentage of default max volume (set at 20% louder)
