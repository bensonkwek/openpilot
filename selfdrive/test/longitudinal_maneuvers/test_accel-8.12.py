...
def run_maneuver_worker(k):
  def run(self):
    man = maneuvers[k]
    print(man.title)
    valid, log = man.evaluate()
    self.assertTrue(valid, msg=man.title)
    with open("testResults.txt", "w") as txt_file:
      txt_file.write("Time, Lead Distance, vEgo, vLead, aEgo\n")
      for line in log:
          # txt_file.write(str(line[0]) + ", " line[0] +  + "\n")
          txt_file.write("%2.2f, %6.2f, %6.2f, %6.2f, %6.2f\n"
            % (line[0], line[2] - line[1], line[3], line[4], line[5]))
  return run
